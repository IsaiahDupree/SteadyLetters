...
export default function ForgotPasswordScreen() {
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      Alert.alert('Check Your Email', `We sent a password reset link to ${email.trim()}.`, [{ text: 'OK', onPress: () => router.back() }]);
    } catch (error) {
    ...
    <KeyboardAvoidingView style={s.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={s.content}>
        ...
        <View style={s.inputWrapper}>
          ...
          <TextInput style={s.input} placeholder="Email" placeholderTextColor={colors.textMuted} value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" autoComplete="email" returnKeyType="done" onSubmitEditing={handleReset} accessibilityLabel="Email address" />
        </View>
        <TouchableOpacity style={[s.button, isLoading && s.buttonDisabled]} onPress={handleReset} disabled={isLoading} accessibilityLabel="Send reset link" accessibilityRole="button">
