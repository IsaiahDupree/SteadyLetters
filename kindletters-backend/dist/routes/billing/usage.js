...
router.get('/', auth_1.authenticateRequest, async (req, res) => {
    try {
        ...
        if (!usage) {
            ...
            usage = await prisma_1.prisma.userUsage.upsert({
        ...
        const planKey = usage.tier;
        const plan = pricing_tiers_1.STRIPE_PLANS[planKey] || pricing_tiers_1.STRIPE_PLANS.FREE;
        // Helper to parse limit string
        const getLimits = (tierName) => {
