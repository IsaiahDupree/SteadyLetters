...
export interface SegmentCondition {
  field: string; // e.g., 'features.daysSinceLastActive', 'person.email'
  operator: 'eq' | 'neq' | 'gt' | 'lt' | 'gte' | 'lte' | 'in' | 'contains';
  value: any;
}

export interface SegmentAction {
  type: 'email' | 'meta_audience' | 'webhook';
 ...
 */
export async function updateSegment(segmentId: string, data: Partial<SegmentData>) {
  return await prisma.segment.update({
    ...
    data: {
      ...
      ...(data.actionConfig && { actionConfig: data.actionConfig as any }),
    },
  });
}
 ...
 */
export async function evaluatePersonAgainstRules(
  personId: string,
  rules: SegmentRules
): Promise<boolean> {
  // Get person data
...
export async function evaluatePersonForSegment(
  ...
  return await evaluatePersonAgainstRules(personId, segment.rules as unknown as SegmentRules);
}
 ...
 */
export async function isPersonInSegment(
  personId: string,
  segmentId: string
): Promise<boolean> {
 ...
 */
export async function updateSegmentMembership(personId: string, segmentId: string) {
  ...
  if (matches && !isCurrentlyMember) {
    ...
    return { isMember: true, action: 'added' as const };
    ...
    return { isMember: false, action: 'removed' as const };
  }
 ...
 */
export async function evaluateSegmentForAllPersons(
  segmentId: string,
  batchSize = 10
): Promise<{
  total: number;
 ...
 */
export async function evaluateAllSegmentsForPerson(personId: string) {
 ...
 */
export async function triggerSegmentAutomation(
  personId: string,
  segmentId: string
): Promise<{ triggered: boolean; type?: string; error?: string }> {
  const segment = await prisma.segment.findUnique({
    where: { id: segmentId },
  });
...
export async function getAllSegmentStatistics() {
  ...
  const stats = await Promise.all(
    segments.map((s) => getSegmentStatistics(s.id))
  );
