...
export interface PersonFeatureData {
  // Behavioral features
  activeDays: number;
  coreActions: number;
  pricingViews: number;
...
export async function computePersonFeatures(
  ...
  const coreActions = person.unifiedEvents.filter((e) =>
    ['letter_sent', 'subscription_started', 'purchase_completed'].includes(
      e.eventName
    )
  ).length;

  // Compute pricing views
  const pricingViews = person.unifiedEvents.filter(
    (e) => e.eventName === 'pricing_view'
  ).length;

  // Compute email opens
  const emailOpens = person.unifiedEvents.filter(
    (e) => e.eventName === 'email_opened'
  ).length;

  // Compute email clicks
  const emailClicks = person.unifiedEvents.filter(
    (e) => e.eventName === 'email_clicked'
  ).length;

  // Product-specific features
  const lettersCreated = person.unifiedEvents.filter(
    (e) => e.eventName === 'letter_created'
  ).length;

  const lettersSent = person.unifiedEvents.filter(
    (e) => e.eventName === 'letter_sent'
  ).length;

  const templatesCreated = person.unifiedEvents.filter(
    (e) => e.eventName === 'template_created'
  ).length;

  const recipientsAdded = person.unifiedEvents.filter(
    (e) => e.eventName === 'recipient_added'
  ).length;
 ...
 */
export async function storePersonFeatures(
  personId: string,
  features: PersonFeatureData
) {
 ...
 */
export async function computeAndStorePersonFeatures(
  personId: string,
  lookbackDays = 90
) {
 ...
 */
export async function computePersonFeaturesBatch(
  personIds: string[],
  batchSize = 10,
  lookbackDays = 90
): Promise<
  Array<{ personId: string; success: boolean; error?: string }>
> {
  const results: Array<{ personId: string; success: boolean; error?: string }> =
    [];
 ...
 */
export async function computeFeaturesForAllPersons(
  batchSize = 10,
  lookbackDays = 90
): Promise<{
  total: number;
 ...
 */
export async function getOrComputePersonFeatures(
  personId: string,
  maxAgeDays = 1,
  lookbackDays = 90
) {
...
export async function getPersonsByFeatures(criteria: {
  ...
  activeDaysMax?: number;
  coreActionsMin?: number;
  coreActionsMax?: number;
  daysSinceLastActiveMin?: number;
  daysSinceLastActiveMax?: number;
  emailOpensMin?: number;
  lettersSentMin?: number;
}): Promise<string[]> {
